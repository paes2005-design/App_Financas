...
@media(max-width:1180px){.dashboard-actions{position:static;transform:none;grid-column:1/-1;justify-content:flex-end;margin-top:8px}.dashboard-toolbar{display:contents}.dashboard-settings-panel{right:0}}
@media(max-width:980px){.dashboard-detail-panel,.movements-panel{grid-column:1/-1}}
@media(max-width:760px){.dashboard-settings-panel{position:fixed;right:12px;left:12px;top:90px;width:auto;max-height:calc(100vh - 110px);overflow:auto}.dashboard-actions{width:100%;justify-content:flex-end}.dashboard-action-button{padding:9px 11px}}
`;
...
if(saldoCard){
  ...
  infoBtn.type="button";infoBtn.className="info-button";infoBtn.textContent="i";infoBtn.setAttribute("aria-label","Detalhar saldo por moeda");
  title.appendChild(infoBtn);
  const info=document.createElement("div");
  info.id="saldoInfoDetalhes";info.className="balance-info hidden";saldoCard.appendChild(info);
...
actions.innerHTML=`
  <button type="button" id="dashboardValueToggle" class="dashboard-action-button" aria-label="Ocultar valores" title="Ocultar valores">◉ Valores</button>
  <div class="dashboard-settings-wrap">
    <button type="button" class="dashboard-action-button dashboard-settings-button" aria-label="Configurar dashboard">⚙ Configurar</button>
    <div class="dashboard-settings-panel hidden">
      <div class="dashboard-settings-head"><strong>Itens da dashboard</strong><button type="button" class="dashboard-settings-back">← Voltar</button></div>
      <label><input type="checkbox" data-panel="movimentacoes" checked> Últimas movimentações</label>
      <label><input type="checkbox" data-panel="receitas" checked> Detalhamento de receitas</label>
      <label><input type="checkbox" data-panel="despesas" checked> Detalhamento de despesas</label>
      <label><input type="checkbox" data-panel="contas"> Detalhamento de contas</label>
      <label><input type="checkbox" data-panel="transferencias"> Transferências</label>
      <label><input type="checkbox" data-panel="cartoes"> Despesas de cartões</label>
      <label><input type="checkbox" data-panel="orcamentos"> Orçamentos</label>
    </div>
  </div>`;

const valueToggle=document.getElementById("dashboardValueToggle");
const settingsWrap=actions.querySelector(".dashboard-settings-wrap");
const settingsButton=settingsWrap.querySelector(".dashboard-settings-button");
const settingsPanel=settingsWrap.querySelector(".dashboard-settings-panel");
const settingsBack=settingsWrap.querySelector(".dashboard-settings-back");

function setValuesHidden(hidden){
  dashboard.classList.toggle("dashboard-values-hidden",hidden);
  valueToggle.textContent=hidden?"◎ Mostrar":"◉ Valores";
  valueToggle.setAttribute("aria-label",hidden?"Mostrar valores":"Ocultar valores");
  valueToggle.title=hidden?"Mostrar valores":"Ocultar valores";
  localStorage.setItem("app-financas-hide-values",hidden?"1":"0");
}
valueToggle.addEventListener("click",()=>setValuesHidden(!dashboard.classList.contains("dashboard-values-hidden")));
setValuesHidden(localStorage.getItem("app-financas-hide-values")==="1");

function openSettings(){
  if(!settingsPanel.classList.contains("hidden"))return;
  settingsPanel.classList.remove("hidden");
  if(!settingsHistoryActive){history.pushState({appOverlay:"dashboard-settings"},"");settingsHistoryActive=true;}
}
function closeSettings({fromHistory=false}={}){
  if(settingsPanel.classList.contains("hidden"))return;
  settingsPanel.classList.add("hidden");
  if(fromHistory){settingsHistoryActive=false;return;}
  if(settingsHistoryActive)history.back();
}
settingsButton.addEventListener("click",()=>settingsPanel.classList.contains("hidden")?openSettings():closeSettings());
settingsBack.addEventListener("click",()=>closeSettings());
window.addEventListener("popstate",()=>{if(!settingsPanel.classList.contains("hidden"))closeSettings({fromHistory:true});});
document.addEventListener("click",event=>{if(!settingsPanel.classList.contains("hidden")&&!settingsWrap.contains(event.target))closeSettings();});

const movementsPanel=lista.closest(".panel");
movementsPanel.dataset.dashboardPanel="movimentacoes";
function createPanel(id,title){
  const panel=document.createElement("article");
  panel.className="panel dashboard-detail-panel";panel.dataset.dashboardPanel=id;
  panel.innerHTML=`<h3>${title}</h3><div id="dashboard-${id}" class="dashboard-detail-list empty-state">Nenhum registro neste mês.</div>`;
  dashboard.appendChild(panel);
}
createPanel("receitas","Detalhamento de receitas");
createPanel("despesas","Detalhamento de despesas");
createPanel("contas","Detalhamento de contas");
createPanel("transferencias","Transferências");
createPanel("cartoes","Despesas de cartões");
createPanel("orcamentos","Orçamentos");
...
function renderList(id,rows,type){
  const el=document.getElementById(`dashboard-${id}`);if(!el)return;
  el.className=rows.length?"dashboard-detail-list":"dashboard-detail-list empty-state";
  el.innerHTML=rows.length?rows.slice(0,10).map(r=>`<div class="history-row"><span>${esc(r.descricao||r.categoriaNome||r.nome||"Registro")} · ${esc(r.data||"")}</span><strong class="${type} financial-value">${type==="receita"?"+":type==="despesa"?"-":""}${formatMoney(r.valor||r.saldoInicial||0,r.moeda||"BRL")}</strong></div>`).join(""):"Nenhum registro neste mês.";
}
...
async function accountBreakdown(){
  ...
  const currencies=await Promise.all([...grouped].map(async([moeda,valor])=>{const converted=await convertAccountsToBRL([{moeda,saldoInicial:valor}]);return{moeda,valor,convertido:converted.total};}));
...
async function render(){
  ...
  lista.innerHTML=movimentos.length?movimentos.map(m=>`<div class="history-row"><span>${esc(m.descricao||m.categoriaNome||"Movimentação")} · ${esc(m.data||"")}</span><strong class="${m._tipo} financial-value">${m._tipo==="receita"?"+":m._tipo==="despesa"?"-":""}${formatMoney(m.valor,m.moeda||"BRL")}</strong></div>`).join(""):"Nenhuma movimentação neste mês.";
}

window.addEventListener("period-change",event=>{if(event.detail.viewId==="dashboard")render();});
onAuthStateChanged(auth,user=>{
  usuario=user;stops.forEach(stop=>stop());stops=[];receitas=[];despesas=[];transferencias=[];contas=[];cartoes=[];orcamentos=[];render();if(!user)return;
  [["receitas",v=>receitas=v],["despesas",v=>despesas=v],["transferencias",v=>transferencias=v],["contas",v=>contas=v],["cartoes",v=>cartoes=v],["orcamentos",v=>orcamentos=v]].forEach(([name,setter])=>{
    const qref=name==="contas"?collection(db,"users",user.uid,name):query(collection(db,"users",user.uid,name),orderBy("data","desc"));
    const stop=onSnapshot(qref,snap=>{setter(snap.docs.map(d=>({id:d.id,...d.data()})));render();},()=>{setter([]);render();});
    stops.push(stop);
  });
});
