...
const nav=document.getElementById("dashboardMonthNavigator");
...
let receitas=[],despesas=[],transferencias=[];
let stops=[];
...
function render(){
  ...
  const totalR=brlTotal(r),totalD=brlTotal(d);
  receitasMes.textContent=formatMoney(totalR,"BRL");
  despesasMes.textContent=formatMoney(totalD,"BRL");
  resultadoMes.textContent=formatMoney(totalR-totalD,"BRL");
...
onAuthStateChanged(auth,user=>{
  stops.forEach(stop=>stop());stops=[];receitas=[];despesas=[];transferencias=[];render();if(!user)return;
  [["receitas",v=>receitas=v],["despesas",v=>despesas=v],["transferencias",v=>transferencias=v]].forEach(([name,setter])=>{
    const stop=onSnapshot(query(collection(db,"users",user.uid,name),orderBy("data","desc")),snap=>{setter(snap.docs.map(d=>({id:d.id,...d.data()})));render();},()=>{setter([]);render();});
    stops.push(stop);
  });
});
