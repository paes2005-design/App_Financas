...
function formatRoot(root){if(!root)return;const walker=document.createTreeWalker(root,4);const nodes=[];while(walker.nextNode())nodes.push(walker.currentNode);for(const n of nodes){const parent=n.parentElement;if(!parent||parent.closest("input,textarea,select,option,script,style"))continue;const next=formatText(n.nodeValue);if(next!==n.nodeValue)n.nodeValue=next;}}
