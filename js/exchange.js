...
export async function getRateToBRL(currency){
  if(!currency || currency==="BRL") return {rate:1,date:new Date().toISOString().slice(0,10),provider:"BRL"};
  ...
  for(const url of urls){
    try{ const response=await fetch(url,{headers:{Accept:"application/json"}}); if(!response.ok) throw new Error(`HTTP ${response.status}`); const data=await response.json(); if(!Number.isFinite(Number(data.rate))) throw new Error("Cotação inválida"); const result={rate:Number(data.rate),date:data.date||"",provider:url.includes("providers=BCB")?"BCB/PTAX":"Frankfurter"}; writeCache(currency,result); return result; }catch(error){ lastError=error; }
  }
...
export async function convertAccountsToBRL(accounts){
  ...
  let total=0; const unavailable=[]; let latestDate="";
  for(const account of accounts){ const currency=account.moeda||"BRL"; const quote=rates.get(currency); if(!quote?.rate){unavailable.push(currency);continue;} total+=Number(account.saldoInicial||0)*quote.rate; if(quote.date>latestDate) latestDate=quote.date; }
  return {total,rates,unavailable:[...new Set(unavailable)],latestDate};
}
