...
const style=document.createElement("style");style.textContent=`.settlement-control{display:inline-flex;align-items:center;gap:6px;margin-right:4px;font-size:.82rem;color:var(--primary);cursor:pointer}.settlement-control input{width:16px;height:16px;accent-color:var(--primary)}.settlement-done{display:inline-flex;align-items:center;gap:5px;margin-right:4px;font-size:.78rem;color:#2e7d32}.settlement-modal{position:fixed;inset:0;z-index:7500;display:grid;place-items:center;padding:18px}.settlement-backdrop{position:absolute;inset:0;background:rgba(15,23,42,.58)}.settlement-card{position:relative;width:min(480px,100%);background:#fff;border-radius:16px;padding:20px;box-shadow:0 24px 70px rgba(0,0,0,.3)}.settlement-card h3{margin:0 0 6px}.settlement-card p{margin:0 0 14px;color:var(--muted);line-height:1.4}.settlement-card label{display:block;margin:12px 0 5px;font-weight:600}.settlement-card input{width:100%;padding:11px;border:1px solid var(--border);border-radius:9px}.settlement-actions{display:flex;gap:9px;margin-top:18px}.settlement-actions button{flex:1;padding:11px;border-radius:9px}.settlement-confirm{border:0;background:var(--primary);color:white;font-weight:700}.settlement-cancel{border:1px solid var(--border);background:white}.settlement-note{display:block;margin-top:8px;color:var(--muted);font-size:.78rem}`;document.head.appendChild(style);
...
async function settle(kind,record,data,valor){const common={categoriaId:record.categoriaId||"",categoriaNome:record.categoriaNome||"",subcategoria:record.subcategoria||"",descricao:record.descricao||"",valor,moeda:record.moeda||"BRL",data,...parts(data),serieId:record.serieId||null,recorrenciaTipo:record.recorrenciaTipo||"unica",competencia:record.competencia||String(record.data||data).slice(0,7),parcelaNumero:record.parcelaNumero||null,parcelasTotal:record.parcelasTotal||null,planejada:false,efetivada:true,dataEfetivacao:data,atualizadoEm:serverTimestamp()};
  if(kind==="receita"||kind==="despesa"){
    ...
    await updateDoc(doc(db,"users",user.uid,kind==="receita"?"receitas":"despesas",record.id),{planejada:false,efetivada:true,dataEfetivacao:data,valorPrevisto:Number(record.valor||0),valorEfetivado:valor,valor,movimentacaoId:mov.id,atualizadoEm:serverTimestamp()});return;
  }
...
document.addEventListener("change",async e=>{const input=e.target.closest("[data-settle-record]");if(!input||!user)return;const kind=input.dataset.settleKind,record=records(kind).find(r=>r.id===input.dataset.settleRecord);if(!record){input.checked=false;return;}const result=await modal(kind,record);if(!result){input.checked=false;return;}const l=labels(kind);if(!confirm(`Confirma ${l.verb} de ${formatMoney(result.valor,record.moeda||"BRL")} em ${new Date(`${result.data}T12:00:00`).toLocaleDateString("pt-BR")}?`)){input.checked=false;return;}try{await settle(kind,record,result.data,result.valor);}catch(error){console.error(error);alert("Não foi possível efetivar a operação.");input.checked=false;}});

onAuthStateChanged(auth,current=>{user=current;stops.forEach(s=>s());stops=[];receitas=[];despesas=[];transferencias=[];decorate();if(!current)return;[["receitas",v=>receitas=v],["despesas",v=>despesas=v],["transferencias",v=>transferencias=v]].forEach(([name,set])=>{stops.push(onSnapshot(query(collection(db,"users",current.uid,name),orderBy("data","desc")),snap=>{set(snap.docs.map(d=>({id:d.id,...d.data()})));decorate();}));});});
