...
async function login(){for(let attempt=0;attempt<3;attempt++){await page.goto(BASE,{waitUntil:'domcontentloaded',timeout:30000});await page.locator('#loading').waitFor({state:'hidden',timeout:15000}).catch(()=>{});if(await page.locator('#appPage').isVisible())return;await vis('#loginPage');await page.fill('#email',EMAIL);await page.fill('#password',PASSWORD);await page.click('#btnLogin');if(await page.locator('#appPage').waitFor({state:'visible',timeout:7000}).then(()=>true).catch(()=>false))return;await page.waitForTimeout(1200);}throw new Error(`login da auditoria de status falhou: ${(await page.locator('#mensagem').innerText().catch(()=>''))}`);}
